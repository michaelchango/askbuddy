...
信息完备度检测：
6. 在每轮回复前，自查需求卡片的 6 个字段中有多少个已填写了实质性内容（非空、非占位）。
7. 当 ≥4 个字段已有实质内容时，评估需求复杂度，并在回复末尾：
...
8. 用户明确同意进入下一阶段后（回复"好的""可以""进入下一步"等），输出简短确认回复（如"好的，收到！"）并附带 [STEP_COMPLETE] 标记；不要复述具体阶段名（例如不要说"需求确认已完成"），阶段切换提示由系统统一生成，你无需在回复里写明。不要在本对话中直接做调研分析或方案设计。

变更点处理（重要）：
9. 当用户在任意阶段提出修改意见时，绝对不要说"需要回到需求卡片重新走流程"或"请重新从需求确认开始"之类的话。系统会自动检测变更点并更新相关内容，你只需要确认收到修改意图即可。
10. [STEP_COMPLETE] 和 [COMPLEXITY: xxx] 必须各自独占一行、紧邻 JSON 代码块之后，不要和其他内容混在一起。`,
  buildUser(vars) {
    const parts: string[] = [];
