 ...
 * - 面板定位：相对于根容器 absolute 定位（默认贴齐右侧）；外层需是 positioned 元素。
 * - 菜单项：悬浮高亮为「内嵌圆角框」（左右留有边距），而非顶到底的整行背景；
 *   文字左对齐，图标与文字 gap-2。
 * - light-dismiss：点击面板外部或按 Esc 都会关闭（自实现）。
 *
...
export function DropdownMenu({
  ...
  // 点击外部或按 Esc 关闭（自实现 light-dismiss，替代之前的 Popover API）。
  useEffect(() => {
    if (!open) return;
    ...
    <div ref={rootRef} className={rootClassName}>
      ...
      {open && (
        <div
          ...
          aria-label={ariaLabel}
          style={{ width }}
          className={`absolute z-30 mt-[6px] overflow-hidden rounded-[10px] border border-[#1111111a] bg-white p-1 shadow-[0_8px_24px_rgba(0,0,0,0.12)] ${
