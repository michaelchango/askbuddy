...
// ---------- 项目 / 需求导航 ----------
server.tool("list_projects", "列出当前 PAT 所属用户的所有项目（id / name / status 等）", {}, async () => {
    const data = await callPlatform(`/api/mcp/projects`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("list_requirements", "列出某项目下的需求（id / title / status）。需传入 projectId。", { projectId: z.string().describe("项目 ID") }, async ({ projectId }) => {
    const data = await callPlatform(`/api/mcp/projects/${projectId}/requirements`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("requirement_get", "获取 AskBuddy 需求的基本信息（需求卡片与步骤状态）", { requirementId: z.string().describe("需求 ID") }, async ({ requirementId }) => {
    const data = await callPlatform(`/api/mcp/requirement/${requirementId}`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
// ---------- 各产物（与需求流程四步对应） ----------
server.tool("requirement_research", "获取 AskBuddy 需求的调研分析结论（Markdown，含报告 / 用户故事 / 功能清单）", { requirementId: z.string().describe("需求 ID") }, async ({ requirementId }) => {
    const data = await callPlatform(`/api/mcp/requirement/${requirementId}/research`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("requirement_solution", "获取 AskBuddy 需求的方案设计文档（Markdown）", { requirementId: z.string().describe("需求 ID") }, async ({ requirementId }) => {
    const data = await callPlatform(`/api/mcp/requirement/${requirementId}/solution`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("requirement_prototype", "获取 AskBuddy 需求的可交互 HTML 原型（含页面结构 JSON）。默认返回完整 HTML。", { requirementId: z.string().describe("需求 ID") }, async ({ requirementId }) => {
    const data = await callPlatform(`/api/mcp/requirement/${requirementId}/prototype?includeHtml=true`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("requirement_prd", "获取 AskBuddy 需求的 PRD 文档（Markdown）。可传 version 读取指定历史版本。", {
...
server.tool("requirement_dev_context", "获取 AskBuddy 需求的整套「开发上下文」DevContext（完整 JSON，含 16 个内容 section、评分与溯源）。供 AI 编码工具直接消费。", { requirementId: z.string().describe("需求 ID") }, async ({ requirementId }) => {
...
server.tool("dev_context_business_rules", "仅取 DevContext 的「业务规则」段（BR-xxx 编号列表，含形式化 when/then 与优先级）。", { requirementId: z.string().describe("需求 ID") }, async ({ requirementId }) => {
    const data = await callPlatform(`/api/mcp/requirement/${requirementId}/dev-context?section=business_rules`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("dev_context_data_structures", "仅取 DevContext 的「数据模型」段（entities 实体字段定义与关系）。", { requirementId: z.string().describe("需求 ID") }, async ({ requirementId }) => {
    const data = await callPlatform(`/api/mcp/requirement/${requirementId}/dev-context?section=data_structures`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("dev_context_api_specs", "仅取 DevContext 的「接口规范」段（REST / 事件接口定义，对应 api_requirements section）。", { requirementId: z.string().describe("需求 ID") }, async ({ requirementId }) => {
    const data = await callPlatform(`/api/mcp/requirement/${requirementId}/dev-context?section=api_requirements`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
server.tool("dev_context_acceptance_criteria", "仅取 DevContext 的「验收标准」段（AC-xxx 编号列表，Given/When/Then 格式，可映射到业务规则）。", { requirementId: z.string().describe("需求 ID") }, async ({ requirementId }) => {
    const data = await callPlatform(`/api/mcp/requirement/${requirementId}/dev-context?section=acceptance_criteria`);
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
});
// ---------- 知识库（M2 占位） ----------
server.tool("search_knowledge", "在 AskBuddy 知识库中检索参考资料（M2 占位：当前返回空结果，待后续版本接入）。", { query: z.string().describe("检索关键词") }, async ({ query }) => {
